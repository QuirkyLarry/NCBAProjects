...
SWEP.Instructions = "Left click to lock pick doors."
SWEP.Purpose = "Lock picks doors"
SWEP.Category = "DarkRP (Raid Tools)"
...
function SWEP:Think()
    ...
    if CurTime() >= self:GetNextSoundTime() then
        ...
        local snd = {1,3,4}
        self:EmitSound("weapons/357/357_reload" .. tostring(snd[math.Round(util.SharedRandom("DarkRP_Shitty_LockpickSnd" .. CurTime(), 1, #snd))]) .. ".wav", 50, 100)
    end
