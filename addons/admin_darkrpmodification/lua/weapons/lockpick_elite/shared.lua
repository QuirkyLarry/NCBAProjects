...
if CLIENT then
    SWEP.PrintName = "Elite Lock Pick"
    SWEP.Slot = 4
    SWEP.SlotPos = 32
    SWEP.DrawAmmo = false
...
function SWEP:Think()
    ...
    if CurTime() >= self:GetNextSoundTime() then
        ...
        local snd = {1,3,4}
        self:EmitSound("weapons/357/357_reload" .. tostring(snd[math.Round(util.SharedRandom("DarkRP_Adv_LockpickSnd" .. CurTime(), 1, #snd))]) .. ".wav", 50, 100)
    end
