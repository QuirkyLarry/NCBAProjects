-- Localized Global Functions, micro-optimizations, can also prevent conflictions.
...
-- Code Section
l_ie("shared.lua")
local matBeam	= l_Material( "cable/xbeam" )
...
function ENT:DrawMainBeam( StartPos, EndPos, dt, dist )
	...
	-- Beam effect
	l_rSM( matBeam )
	l_rDB( EndPos, StartPos,2,TexOffset * -0.4, TexOffset * -0.4 + StartPos:DistToSqr(sqEndPos) / 256, ca )
end
