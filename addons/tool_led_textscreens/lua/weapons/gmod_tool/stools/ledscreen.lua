...
if (SERVER) then
	CreateConVar('sbox_maxledscreens', 3)
end
...
function TOOL:LeftClick(tr)
	...
	local TextScreen = ents.Create("gb_rp_sign")
	...
	hook.Run('SammyTextScreen_Placed', Ply, self:GetClientInfo("text"))
...
function TOOL:RightClick(tr)
	...
	if (TraceEnt:IsValid() and TraceEnt:GetClass() == "gb_rp_sign") then
		...
		hook.Run('SammyTextScreen_Placed', self:GetOwner(), self:GetClientInfo("text"))
		return true
	end
