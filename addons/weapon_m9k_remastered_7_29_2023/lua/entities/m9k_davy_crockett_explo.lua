...
if SERVER then
	...
	local tWhitelist = { -- List of blacklisted entities, traces cannot go through these.
		["mmm_object_detail"] = false -- Protective shield.
	}
	...
	local fLoSCheck = function(eNuke,eTarget,vTarget)
		...
		if !tWhitelist[tTraceRes.Entity ~= NULL and tTraceRes.Entity:GetClass()] then -- Nope!
			return false
		end
	...
	function ENT:Initialize()
		...
		for _,v in ipairs(ents.FindInSphere(self.SplodePos,iBlastRadius)) do
			...
			if tEntity.bValid and tEntity.bPlayer then
	...
	function ENT:Think()
		...
		--[[for _,v in ipairs(ents.FindInSphere(self.SplodePos,self.SplodeDist)) do -- Entities returned by this cannot be invalid.
			...
			local tEntity = fGetEntityInfo(v)
			if not tEntity.bValid or not tEntity.bPlayer then -- ... But we have our custom valid check!
