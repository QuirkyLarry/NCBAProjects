...
if SERVER then
	...
	function SWEP:Initialize()
		...
		if ConVar_SpawnReserve:GetInt() == 1 then -- Give the Weapon Owner some clips if desired
			...
			timer.Simple(0,function() -- We need to wait one tick so that self:GetOwner() becomes valid
				if not IsValid(self) or not IsValid(self:GetOwner()) then return end

				self:GetOwner():GiveAmmo(self.Primary.ClipSize * 3,self.Primary.Ammo,true) -- 3 Clips
			end)
		end
		...
		timer.Simple(0.1,function() -- Pray the weapon is valid on the client on runtime
			if not IsValid(self) or not IsValid(self:GetOwner()) or self:GetOwner():GetActiveWeapon() ~= self then return end
	...
	function SWEP:ResetInternalVars() -- Convenience
		if self.IronSightState and self:GetOwner() and IsValid(self:GetOwner()) then
			self:GetOwner():SetFOV(0,0.3) -- Setting this at all times is bad as it causes screen spazz
		end
	...
	function SWEP:Equip()
		...
		if not self:GetOwner():IsPlayer() then -- NPCs cannot have M9kR Weapons! Also prevents invalid spawning
			self:Remove()
	...
	function SWEP:PrimaryAttack()
		if not IsValid(self:GetOwner()) then return end -- If somehow an NPC got it on their hands or similar.. this will cause huge problems!
		...
		if self:GetOwner():WaterLevel() == 3 and not self.CanFireUnderwater then -- Cannot fire underwater
			self:GetOwner():EmitSound("weapons/masadamagpul/safety.mp3")
		...
		if not self.bFiresEntity then
			...
			local iSpread = (not self.ShouldDoMoveSpread and self.Primary.Spread) or (self:GetOwner():GetVelocity():Length() > 100 and self.Primary.Spread * 2) or (self:GetOwner():KeyDown(IN_DUCK) and self.Primary.Spread / 2) or self.Primary.Spread


			self:GetOwner():LagCompensation(true) -- Apparently this is called internally by FireBullets, however I get way better results with a high ping when this is called beforehand which means it might not be called internally at all
		...
		local bDuck = self:GetOwner():KeyDown(IN_DUCK)

		local KickUp			= 0 --(not bDuck and self.Primary.KickUp			or self.Primary.KickUp / 4)
		local KickDown			= 0 --(not bDuck and self.Primary.KickDown			or self.Primary.KickDown / 4)
		local KickHorizontal	= 0 --(not bDuck and self.Primary.KickHorizontal	or self.Primary.KickHorizontal / 4)


		--local aRecoil = Angle(math.random(-KickDown,-KickUp),math.random(-KickHorizontal,KickHorizontal),0)

		--self:GetOwner():ViewPunch(aRecoil)
	...
	function SWEP:Reload()
		...
		if self:GetOwner():GetAmmoCount(self.Primary.Ammo) >= 1 or self:Clip1() >= 1 then
		...
		if self:GetNextPrimaryFire() < CurTime() and self:GetOwner():GetAmmoCount(self.Primary.Ammo) <= 0 then
			self:GetOwner():EmitSound("weapons/masadamagpul/safety.mp3")
	...
	function SWEP:IronSight()
		if self:GetOwner():GetViewEntity() ~= self:GetOwner() or self.DynamicReloadThink then return end
		...
		if self:GetOwner():KeyPressed(IN_ATTACK2) and not self.IronSightState then
			self.IronSightState = true

			self:GetOwner():SetFOV(80,0.2)
		elseif self:GetOwner():KeyReleased(IN_ATTACK2) then
			self.IronSightState = false
	...
	function SWEP:Think()
		...
		if self.DynamicReloadThink and type(self.DynamicReloadThink) == "number" and CurTime() > self.DynamicReloadThink then -- Number check is required for Singleplayer compatibility
			...
			self:SetClip1(self:GetOwner():GetAmmoCount(sType) >= iAmount and iAmount or self:GetOwner():GetAmmoCount(sType))

			self:GetOwner():SetAmmo(self:GetOwner():GetAmmoCount(sType) - iAmount,sType)
...
if CLIENT then
	...
	function SWEP:ResetInternalVars() -- Convenience
		if self.IronSightState and IsValid(self:GetOwner()) then
			self:GetOwner():SetFOV(0,0.3) -- Setting this at all times is bad as it causes screen spazz
		end
	...
	function SWEP:PrimaryShootEffects(bBool)
		if (not bBool and self:GetOwner() == LocalPlayer()) then return end -- It already played for them!


		self:EmitSound(self.Primary.Sound,50,math.random(95,105))
		...
		if IsValid(self:GetOwner()) then
			...
			--self:GetOwner():ViewPunch(aRecoil) -- Not needed.


			local aEyes = self:GetOwner():EyeAngles()
				aEyes:SetUnpacked(aEyes.p + aRecoil.p,aEyes.y + aRecoil.y,0)

			self:GetOwner():SetEyeAngles(aEyes)



			--local vm = self:GetOwner():GetViewModel() -- All of this is commented out for now since not restricting it seems to be better.

			--if not IsValid(vm) or (IsValid(vm) and self:GetOwner():GetViewEntity() ~= self:GetOwner()) then -- Everyone except the Owner when they are in first person
				...
				if obj_DynLight then
					...
					obj_DynLight.pos = self:GetOwner():GetShootPos() + self:GetOwner():EyeAngles():Forward() * 30
	...
	function SWEP:ReloadDynamic(bBool)
		...
		if self.tReloadDynamic then
			...
			hook.Add("Tick",sStr,function() -- The fact I have to do this is just sad. Improvising a Think here is.. needed
				...
				if self.IsReloading and IsValid(self) and IsValid(self:GetOwner()) then
					...
					for k,v in ipairs(self.tReloadDynamic) do
						...
						if iPassed > v.iDelay then
							...
							self:GetOwner():EmitSound(v.sSound,v.iVolume or 70,math.random(95,105),1,CHAN_ITEM) -- Random pitch = Better
	...
	function SWEP:CanPrimaryAttack()
		...
		if self:GetNextPrimaryFire() > CurTime() or (self:GetOwner():GetAmmoCount(self.Primary.Ammo) <= 0 and self:Clip1() <= 0) then
			return false
		end
	...
	function SWEP:IronSight()
		if self:GetOwner():GetViewEntity() ~= self:GetOwner() or self.IsReloading then return end
	...
	function SWEP:FireAnimationEvent(_,_,event) -- Shell ejections
		if event == 5001 or event == 5011 or event == 5021 or event == 5031 then
			if self:GetOwner():GetViewEntity() ~= self:GetOwner() then return true end
